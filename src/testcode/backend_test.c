...
int main(void) {
  ...
  // Add games
  ErrorCode add_err;
  add_err = addGame(shop, "Tetris", 9.99);
  printf("Add Tetris: %s\n", add_err == SUCCESS ? "SUCCESS" : "FAILED");

  add_err = addGame(shop, "Wormgame", 4.50);
  printf("Add Wormgame: %s\n", add_err == SUCCESS ? "SUCCESS" : "FAILED");

  // Test duplicate addition
  add_err = addGame(shop, "Tetris", 9.99);  // Should fail
  ...
  ErrorCode buy_err;
  buy_err = buyGame(shop, "Tetris", 3);
  printf("Buy Tetris x3: %s\n", buy_err == SUCCESS ? "SUCCESS" : "FAILED");

  buy_err = buyGame(shop, "NonExistent", 1);  // Should fail
  ...
  printShop(empty_shop);
  printf("Empty shop revenue:\n");
  printRevenue(empty_shop);
  freeShop(empty_shop);

  // Test individual functions
  printf("\n7. Individual Function Tests\n");
  Game* test_game = createGame("TestGame", 10.0, &add_err);
  if (test_game) {
  ...
  // Test tree size
  if (shop) {
    size_t size = treeSize(shop->root);
